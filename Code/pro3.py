...
class ParticleSwarmOptimization:
    ...
    def initialize_swarm(self):
        ...
        for particle_index in range(len(self.positions)):
            # 初始化第一次投放时间变量对应的位置
            self.positions[particle_index][2] = np.random.uniform(0, T)
            # 初始化第一次投放时间变量对应的速度
            self.velocities[particle_index][2] = np.random.uniform(-0.1 * T, 0.1 * T)
            
            # 初始化第二次投放时间变量对应的位置
            self.positions[particle_index][3] = np.random.uniform(self.positions[particle_index][2]  + 1, T)
            # 初始化第二次投放时间变量对应的速度
            A = T - self.positions[particle_index][2] - 1
            self.velocities[particle_index][3] = np.random.uniform(-0.1 * A, 0.1 * A)
            
            # 初始化第三次投放时间变量对应的位置
            self.positions[particle_index][4] = np.random.uniform(self.positions[particle_index][3]  + 1, T)
            # 初始化第三次投放时间变量对应的速度
            A = T - self.positions[particle_index][3] - 1
            self.velocities[particle_index][4] = np.random.uniform(-0.1 * A, 0.1 * A)

        # 初始化三个引爆时间决策变量
        for particle_index in range(len(self.positions)):
            # 初始化第一次引爆时间变量对应的位置
            self.positions[particle_index][5] = np.random.uniform(self.positions[particle_index][2], T)
            # 初始化第一次引爆时间变量对应的速度
            A = T - self.positions[particle_index][2]
            self.velocities[particle_index][5] = np.random.uniform(-0.1 * A, 0.1 * A)
            
            # 初始化第二次引爆时间变量对应的位置
            self.positions[particle_index][6] = np.random.uniform(self.positions[particle_index][3], T)
            # 初始化第二次引爆时间变量对应的速度
            A = T - self.positions[particle_index][3]
            self.velocities[particle_index][6] = np.random.uniform(-0.1 * A, 0.1 * A)
            
            # 初始化第三次引爆时间变量对应的位置
            self.positions[particle_index][7] = np.random.uniform(self.positions[particle_index][4], T)
            # 初始化第三次引爆时间变量对应的速度
            A = T - self.positions[particle_index][4]
    ...
    def update_velocities(self, iter):
        ...
        for particle_index in range(len(self.positions)):
            ...
            A_deli_2 = T - self.positions[particle_index][2] - 1
            ...
            A_deli_3 = T - self.positions[particle_index][3] - 1
            ...
            A_bomb_1 = T - self.positions[particle_index][2]
            ...
            A_bomb_2 = T - self.positions[particle_index][3]
            ...
            A_bomb_3 = T - self.positions[particle_index][4]
    ...
    def update_positions(self):
        ...
        for particle_index in range(len(self.positions)):
            self.positions[particle_index][2] = np.clip(self.positions[particle_index][2], 0, T)
    ...
    def gen_new_particle(self):
        ...
        # 初始化三个投放时间决策变量
        # 初始化第一次投放时间变量对应的位置
        position[2] = np.random.uniform(0, T)
        # 初始化第二次投放时间变量对应的位置
        position[3] = np.random.uniform(position[2]  + 1, T)
        # 初始化第三次投放时间变量对应的位置
        position[4] = np.random.uniform(position[3]  + 1, T)
            

        # 初始化三个引爆时间决策变量
        # 初始化第一次引爆时间变量对应的位置
        position[5] = np.random.uniform(position[2], T)
        # 初始化第二次引爆时间变量对应的位置
        position[6] = np.random.uniform(position[3], T)
        # 初始化第三次引爆时间变量对应的位置
        position[7] = np.random.uniform(position[4], T)
